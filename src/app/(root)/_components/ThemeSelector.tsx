  ...
  return (
    <div className="relative" ref={dropdownRef}>
      <motion.button
        ...
        className="w-48 group relative flex items-center gap-4 px-4 py-2.5 bg-[#1e1e2e]/80 hover:bg-[#262637] rounded-lg transition-all duration-200 border border-gray-800/50 hover:border-gray-700"
      >
        ...
        <Palette className="size-5 text-gray-400 group-hover:text-gray-300 transition-colors" />
        <span className="text-gray-300 text-sm min-w-20 text-left group-hover:text-white transition-colors">
          {currentTheme?.label}
        </span>
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="absolute top-full left-0 mt-2 w-full min-w-60 bg-[#1e1e2e]/95 backdrop-blur-xl rounded-xl border border-[#313244] shadow-2xl py-2 z-50"
          >
            ...
            {THEMES.map((t, index) => (
              ...
              >
                ...
                {/* icon */}
                <div
                  className={`flex items-center justify-center size-6 rounded-lg ${theme === t.id ? "bg-blue-500/10 text-blue-400" : "bg-gray-800/50 text-gray-400"} group-hover:scale-110 transition-all duration-200`}
                >
